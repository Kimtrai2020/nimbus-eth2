import std/options,
       ...
       "."/[rest_constants, rest_traversals_cache]
...
template withStateForBlockSlot*(nodeParam: BeaconNode,
  ...
  block:
    ...
    if isState(node.dag.headState):
      ...
      let cachedTraversalState = if node.restTraversalsCache != nil:
        node.restTraversalsCache.getClosestState(blockSlot.slot)
      else:
        nil

      let stateToAdvance = if cachedTraversalState != nil:
        cachedTraversalState
      else:
        assignClone(node.dag.headState)

      node.dag.updateStateData(stateToAdvance[], blockSlot, false, cache)

      let providedState = if cachedTraversalState == nil:
        # This was not a cached state, we can cache it now
        node.restTraversalsCache.add(stateToAdvance)
        stateToAdvance
      else:
        # If we have a cache hit, there is a concern that the REST request
        # handler may continue executing asyncronously while we hit the same
