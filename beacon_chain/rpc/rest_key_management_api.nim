...
import ".."/".."/beacon_chain/validators/slashing_protection
import ".."/validators/slashing_protection_common
...
proc checkAuthorization*(request: HttpRequestRef, node: BeaconNode): Result[void, AuthorizationStatus] =
    ...
    for authHeader in authorizations:
      ...
      if parts.len == 2 and parts[0] == "Bearer":
        if parts[1] == node.keymanagerToken.get:
          ...
          return err AuthorizationStatus.incorrectToken
    return err AuthorizationStatus.missingBearerScheme
  else:
    return err AuthorizationStatus.noAuthorizationHeader
...
proc installKeymanagerHandlers*(router: var RestRouter, node: BeaconNode) =
  ...
  router.api(MethodGet, "/api/eth/v1/keystores") do () -> RestApiResponse:
    let security = checkAuthorization(request, node)
    if security.isErr():
