...
proc isEnabled*(validatorsDir, keyName: string): bool {.
  ...
  let disableFile = keystoreDir / DisableFileName
...
proc createValidatorFiles* (secretsDir, validatorsDir,
                          keystoreDir,
                          secretFile, passwordAsString,
                          keystoreFile, encodedStorage: string): Result[void, KeystoreGenerationError]  =
...
proc saveKeystore*(rng: var BrHmacDrbgContext,
                   ...
                   signingKeyPath: KeyPath, password: string): Result[void, KeystoreGenerationError] =
  let
